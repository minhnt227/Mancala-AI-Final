...
class MancalaTkinterUI(tk.Tk):
    def __init__(
        ...
        self.player_1_button = tk.Button(
            ...
            pady=self.reference_size // 2,
            font=self.font_med,
            command=lambda: self.choose_player(player_type=1),
        )
        ...
        self.player_2_button = tk.Button(
            ...
            pady=self.reference_size // 2,
            font=self.font_med,
            command=lambda: self.choose_player(player_type=2),
        )
            ...
            for column in range(6):
                ...
                mancala_button = tk.Button(
                    ...
                    command=choose_move_closure(
                        location="top" if row == 0 else "bot",
    ...
    def sync_backend_front_end(self):
        ...
        for index, tile in enumerate(self.board["bot"]):
    ...
    def choose_player(self, player_type: int):
    ...
    def move_pieces(self, turn: str, tile: int):
        ...
        while pieces > 0:
            ...
            location = "bot" if location == "top" else "top"
            tile = -1
        ...
        inverse_location = "bot" if location == "top" else "top"
        if (
            (location == turn)
        ...
        if (not any(self.board["top"])) or (not any(self.board["bot"])):
            self.board["top_score"] += sum(self.board["top"])
            self.board["bot_score"] += sum(self.board["bot"])

            self.board["top"] = [0] * len(self.board["top"])
            self.board["bot"] = [0] * len(self.board["bot"])
